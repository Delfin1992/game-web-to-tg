...
export async function handlePvpMessage(input: {
  ...
  if (command === "/pvp_find") {
    ...
    try {
      const previousResult = await callInternalApi("POST", "/api/pvp/result/claim", { userId: player.id }) as any;
      if (previousResult?.result) {
        previousResult.result.userId = player.id;
        previousResult.result.userName = "Ты";
        stopPvpQueuePolling(chatId);
        await sendMessage(token, chatId, formatPvpResultText(previousResult.result), {
  ...
  if (command === "/pvp_leave") {
    const player = await resolveOrCreateTelegramPlayer(message.from);
    const pvpLockState = await getCurrentPvpLockState(player.id);
    if (pvpLockState === "duel_active") {
      await sendWithCurrentHubKeyboard(token, chatId, player.id, "⛔ PvP-дуэль уже идёт. Сначала закончи бой и потом выходи.");
      return true;
    }
    try {
